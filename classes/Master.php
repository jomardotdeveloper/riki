...
class Master extends DBConnection
{
	...
	function save_to_cart()
	{
		...
		$check = $this->conn->query("SELECT * FROM `cart_list` where client_id = '{$client_id}' and product_id = '{$product_id}' and is_origin = '{$is_origin}'")->num_rows;
		if ($check > 0) {
			$sql = "UPDATE `cart_list` set quantity = quantity + {$quantity}  where product_id = '{$product_id}' and client_id = '{$client_id}' and is_origin = '{$is_origin}'";
		} else {
			$sql = "INSERT INTO `cart_list` set quantity = quantity + {$quantity}, product_id = '{$product_id}', client_id = '{$client_id}', is_origin = '{$is_origin}'";
		}
		$save = $this->conn->query($sql);
		if ($save) {
			$resp['status'] = 'success';
	...
	function update_cart_quantity()
	{
		...
		if ($get["is_origin"] == 0) {
			if ($get["quantity"] <= 1 && str_contains($quantity, "-")) {
				$resp['status'] = 'failed';
				$resp['msg'] = " You are at the lowest quantity.";
			} else {
				$resp['status'] = 'success';
				$resp["message"] = $quantity;
		...
		$out = $this->conn->query("SELECT SUM(quantity) FROM order_items where product_id = '{$pid}' and order_id in (SELECT id FROM order_list where `status` != 5) and is_origin=1")->fetch_array()[0];
	...
	function place_order_other_shop()
	{
	}
	...
	function place_order()
	{
		...
		if ($save) {
			...
			while ($row = $cart->fetch_assoc()) {
				if (!empty($data)) $data .= ", ";
				$data .= "('{$oid}','{$row['product_id']}','{$row['quantity']}', '{$row['is_origin']}')";
				$total_amount += ($row['price'] * $row['quantity']);

				if ($row['is_origin'] == 1) {
					$total_amount_origin += ($row['price'] * $row['quantity']);
				} else {
					$total_amount_non += ($row['price'] * $row['quantity']);
				}
			}
			if ($total_amount_origin <= 0) {
				$total_amount_origin = null;
			}
			if ($total_amount_non <= 0) {
				$total_amount_non = null;
			}
			if (!empty($data)) {
				$sql2 = "INSERT INTO `order_items` (`order_id`,`product_id`,`quantity`, `is_origin`) VALUES {$data}";
				$save2 = $this->conn->query($sql2);
				if ($save2) {
					$resp['status'] = 'success';
					$this->conn->query("DELETE FROM `cart_list` where client_id = '{$client_id}'");
					$this->conn->query("UPDATE `order_list` set total_amount = '{$total_amount}', total_origin = '{$total_amount_origin}', total_non = '{$total_amount_non}' where id = '{$oid}'");
